...
owner: Кристина
related: ../../product-specs/index.md, roadmap.md
---
...
#### A.1. Продуктовые спеки (Фаза 1 / Track A)
- [ ] **3 стартовых упражнения** для захардкоженной тренировки → раздел в `product-specs/exercises-base.md`
  - [ ] Какие именно три упражнения (например, приседания, жим лёжа, тяга — финальный выбор)
  - [ ] Что хранится по каждому: название, краткое описание техники, ключевые ошибки, картинка (опционально)
  - [ ] Источник описаний техники (пишем сами / лицензируем / консультант)
- [ ] **Захардкоженная единственная тренировка** из этих трёх упражнений → раздел в `product-specs/workout.md`
  - [ ] Список упражнений в фиксированном порядке
  - [ ] Что показывается на экране тренировки (название, список упражнений, тапаются)

#### A.2. User flow Single-scenario MVP (Фаза 1 / Track A)
- [ ] **Главный (и единственный) сценарий MVP** — загрузка видео и получение разбора → `user-flows/upload-video-and-get-feedback.md`
  - [ ] От «открыл приложение» до «получил ответ AI и задал уточняющий вопрос»
  - [ ] Тап по упражнению на экране тренировки → **сразу попадание в чат** этого упражнения (без промежуточных экранов)
  - [ ] Загрузка видео из чата (аттач-кнопка) → **только выбор готового файла из галереи устройства** (системный пикер), **без съёмки видео в приложении** → отправка → ответ AI приходит в этот же чат
  - [ ] Поведение при «AI ещё разбирает» (заблокированное поле ввода, индикатор внутри чата)
  - [ ] Поведение при ошибке загрузки / плохом видео / нерелевантном упражнении
